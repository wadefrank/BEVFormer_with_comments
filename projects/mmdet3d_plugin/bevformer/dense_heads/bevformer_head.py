...
class BEVFormerHead(DETRHead):
    ...
    def loss_single(self,
        ...
        """
        # cls_scores.shape = torch.Size([1, 900, 10])
        # num_imgs = 1
        num_imgs = cls_scores.size(0)

        # cls_scores_list(type = <class 'list'>, len = 1)
        # cls_scores_list[0].shape = torch.Size([900, 10])
        cls_scores_list = [cls_scores[i] for i in range(num_imgs)]

        # bbox_preds_list(type = <class 'list'>, len = 1)
        # bbox_preds_list[0].shape = torch.Size([900, 10])
        bbox_preds_list = [bbox_preds[i] for i in range(num_imgs)]


        # 1.按照batch处理，计算分类和回归的targets
        # cls_reg_targets(type = <class 'tuple'>, len = 6)
        # cls_reg_targets[0](type = <class 'list'>, len = 1) cls_reg_targets[0][0].shape = torch.Size([900])
        # cls_reg_targets[1](type = <class 'list'>, len = 1) cls_reg_targets[1][0].shape = torch.Size([900])
        # cls_reg_targets[2](type = <class 'list'>, len = 1) cls_reg_targets[2][0].shape = torch.Size([900, 9])
        # cls_reg_targets[3](type = <class 'list'>, len = 1) cls_reg_targets[3][0].shape = torch.Size([900, 10])
        # cls_reg_targets[4](type = <class 'int'>) 45
        # cls_reg_targets[5](type = <class 'int'>) 855
        cls_reg_targets = self.get_targets(cls_scores_list, bbox_preds_list,
                                           gt_bboxes_list, gt_labels_list,
                                           gt_bboxes_ignore_list)
        
        # labels_list[0].shape = torch.Size([900])
        # label_weights_list[0].shape = torch.Size([900])
        # bbox_targets_list[0].shape = torch.Size([900, 9])
        # bbox_weights_list[0].shape = torch.Size([900, 10])
        # num_total_pos = 45
        # num_total_neg = 855
        ...
        # label_weights.shape = torch.Size([900])
        label_weights = torch.cat(label_weights_list, 0)

        # bbox_targets.shape = torch.Size([900, 9])
        bbox_targets = torch.cat(bbox_targets_list, 0)

        # bbox_weights.shape = torch.Size([900, 10])
        ...
        # cls_avg_factor = 45 * 1.0 + 855 * 0 = 45.0
